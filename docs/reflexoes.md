...
Na Aula 5, o cálculo de multa foi implementado por meio de herança, com cada subtipo de equipamento possuindo sua própria implementação do método `calcular_multa`. Embora essa solução atendesse ao princípio Open/Closed (OCP), ela vinculava a política de cálculo ao tipo de equipamento. Com a aplicação do padrão Strategy, o algoritmo de multa passou a ser representado por objetos independentes, que podem ser trocados sem modificar a classe `Equipamento`. Dessa forma, a composição oferece maior flexibilidade do que a herança, pois permite criar novas políticas de cálculo sem alterar a hierarquia existente. Assim, o OCP não foi perdido; ele continua sendo atendido, mas agora por meio da extensão de estratégias em vez da criação de subclasses.

A aplicação do padrão Observer também trouxe melhorias importantes. Antes, o `ServicoEmprestimo` dependia diretamente de um notificador específico, ficando responsável tanto pelas regras de negócio quanto pelo envio de notificações. Após a refatoração, o serviço passou apenas a emitir eventos, enquanto os observadores registrados decidem como reagir a eles. Isso melhora o SRP, pois cada classe possui uma única responsabilidade, o OCP, pois novos tipos de observadores podem ser adicionados sem modificar o serviço, e o DIP, pois o serviço depende apenas da abstração `Observer`. Os eventos foram representados por dicionários para simplificar a implementação e destacar o funcionamento do padrão. Entretanto, essa decisão introduz o smell conhecido como Primitive Obsession, já que as chaves e valores não possuem tipagem explícita. Mesmo assim, foi uma escolha consciente para manter o foco da atividade no padrão Observer, deixando a refatoração para uma estrutura mais adequada como trabalho futuro.

## Aula 12 — Refactoring e Code Smells

A suíte de testes funcionou como uma rede de segurança durante toda a atividade. Após cada refactoring realizado no projeto, executei `pytest -v` para verificar se o comportamento observável continuava preservado. Os 27 testes permaneceram passando, o que autorizou a continuidade para o próximo passo. Isso foi importante principalmente nas alterações do `ServicoEmprestimo`, pois mudanças como a substituição do dicionário por `Evento` e a extração da notificação de atraso poderiam introduzir erros sem alterar imediatamente a aparência do código. Conforme Valente, Cap. 9, refactoring consiste em melhorar a estrutura interna do software sem alterar seu comportamento observável. Sem testes, seria muito mais difícil ter segurança de que uma melhoria estrutural não havia introduzido uma regressão.

Um falso positivo identificado foi o das subclasses `Notebook`, `Projetor` e `Cabo` em `models/equipamento.py`. Elas ficaram sem métodos próprios após a aplicação do Strategy, podendo aparentar uma estrutura desnecessária ou uma Data Class sem comportamento. Entretanto, elas representam intencionalmente diferentes tipos de equipamento e são utilizadas pela fábrica. Portanto, não devem ser eliminadas. Aplicar `Inline Class` faria essas subclasses desaparecerem e poderia desfazer a separação obtida com Strategy e OCP na Aula 11. Assim, reconhecer um possível smell e decidir não refatorá-lo também faz parte de uma boa análise de código.
